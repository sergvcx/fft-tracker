...
;
	...
	//nm8u    *wantedImage8   = nmppsMalloc_8u(size);
	...
	nm32fcr *tmpFFT_fcr		= (nm32fcr *)ippMalloc(3 * size * sizeof(nm32fcr));
	...
	VS_CreateSlider("wantedSize", 1, 1, 255, 1, 37);
	VS_CreateSlider("blurSize", 2, 1, 255, 1, 37);
	...
	while (status=VS_Run()) {
		...
		if (1) {
			...
			for (int i = 0; i < dim; i++) {
				...
				//nmppsFFT64Fwd_32fcr((nm32fcr*)currImage_fcr + i * dim, (nm32fcr*)tmpFFT_fcr + i, spec64);
				//
				//ippsFFTFwd_CToC_32fc(currImage_fc + i * dim, (Ipp32fc*)tmpFFT_fc + i, sp32, buffer);
				//nmppsFFT32Fwd_32fcr((nm32fcr*)currImage_fcr + i * dim, (nm32fcr*)tmpFFT_fcr + i, spec32);
				//
				//ippsFFTFwd_CToC_32fc(currImage_fc + i * dim, (Ipp32fc*)tmpFFT_fc + i, sp16, buffer);
				//nmppsFFT16Fwd_32fcr((nm32fcr*)currImage_fcr + i * dim, (nm32fcr*)tmpFFT_fcr + i, spec16);
				//
				//ippsFFTFwd_CToC_32fc(currImage_fc + i * dim, (Ipp32fc*)tmpFFT_fc + i, sp8, buffer);
				//nmppsDFT8Fwd_32fcr((nm32fcr*)currImage_fcr + i * dim, (nm32fcr*)tmpFFT_fcr + i, spec16);
		...
		nmppsSub_32s(currImage32s, prevImage32s + caughtX- wantedX + (caughtY-wantedY) * dim, diffImage32s, dim*dim);
		...
		dimToOrg(caughtX, caughtY, caughtOrgX, caughtOrgY, dim, orgSize, cropFactor);
		dimToOrg(caughtX+wantedSize, caughtY+ wantedSize, caughtOrgXX, caughtOrgYY, dim, orgSize, cropFactor);
		...
		VS_Rectangle(CURR_IMG8,   caughtX-1, caughtY-1, caughtX + wantedSize, caughtY + wantedSize, VS_GREEN, VS_NULL_COLOR);
		VS_Rectangle(IFFT_IMG, caughtX-1, caughtY-1, caughtX + wantedSize, caughtY + wantedSize, VS_GREEN, VS_NULL_COLOR);
		VS_Rectangle(CURR_IMG_FC, caughtX-1, caughtY-1, caughtX + wantedSize, caughtY + wantedSize, VS_GREEN, VS_NULL_COLOR);
