...
//////  
  ...
  fpu 0 rep 1 vreg7 = [ar5];//load 32 to add with farther indexses 
  nop;

///init
  fpu  rep 32 .packer = [ar0++gr0] with .double <= .float .in_high;
  fpu 1 rep 32 vreg0 = .packer;//load x0
  //fpu 1 .double vreg0 = /vreg0/;
  gr5--;
  if =0 goto REMINDER;
 <LOOP> 
  fpu  rep 32 .packer = [ar0++gr0] with .double <= .float .in_high;
  fpu 1 rep 32 vreg1 = .packer;//load x1
...
<REMINDER>
  ...
  ar5 = buff;
////aligting
  fpu  rep vlen .packer = [ar0++gr0] with .double <= .float .in_high;
  fpu 0 rep vlen vreg6 = .packer;//load x
